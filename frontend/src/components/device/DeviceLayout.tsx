...
export default function DeviceLayout({
  ...
  selectedDevice: DeviceInSummary;
  setShowSensor: React.Dispatch<React.SetStateAction<boolean>>;
  setSelectedSensor: React.Dispatch<React.SetStateAction<any | null>>;
}) {
  const { sendRequest: powerToggleRequest } = useApi();
  const summary = useAppSelector((store) => store.summary);
  const dispatch = useAppDispatch();
  const [loading, setLoading] = useState<boolean>(false);
  const [loadingBar, setLoadingBar] = useState(Array.from({ length: 48 }));
  const rectRefs = useRef<SVGRectElement[]>([]);
  useEffect(() => {
    if (!loading) {
      rectRefs.current.forEach((rect) => {
        rect.classList.remove("fill-orange-500");
        rect.classList.add("fill-[#4d4d4d]");
      });
      return;
    }
    let index = 0;
    const interval = setInterval(() => {
      rectRefs.current[index]?.classList.remove("fill-[#4d4d4d]");
      rectRefs.current[index]?.classList.add("fill-orange-500");
      index++;
      if (index == 38) {
        handlePowerToggle();
      }
      if (index >= rectRefs.current.length) {
        index = 0;
        // Reset all
        rectRefs.current.forEach((rect) => {
          rect.classList.remove("fill-orange-500");
  ...
  const handlePowerToggle = async () => {
    try {
      powerToggleRequest(`api/device/powerToggle/${selectedDevice._id}`).then(
        (result) => {
          const data = result.data as Data<null> | undefined;
          if (result && result.success && data && data.status) {
            dispatch(devicePowerToggle(selectedDevice._id));
          } else {
  ...
  return deviceReference ? (
    ...
    `}</style>
        ...
        />
        {loadingBar.map((item, oi) => {
          // left
          if (oi <= 18) {
            return (
              <g
                key={`loadingbar/${oi}`}
                transform={`translate(${oi == 0 ? 0.53 : 0.53 - oi * 18} 235.48)`}
              >
                <rect
                  ref={(el) => {
                    if (el) rectRefs.current[oi] = el;
                  }}
                  width={13.155}
                  height={27.996}
                  x={167.621}
          ...
          if (oi > 18 && oi <= 37) {
            return (
              <g
                key={`loadingbar/${oi}`}
                transform={`translate(${oi == 19 ? -321.204 : -321.204 + (oi - 19) * 18})`}
              >
                <rect
                  ref={(el) => {
                    if (el) rectRefs.current[oi] = el;
          ...
          if (oi > 37 && oi < loadingBar.length) {
            return (
              <g
                ...
                transform={`matrix(0 -.79384 .79384 0 116.742 ${oi == 38 ? 166.816 : 166.816 + (oi - 38) * 18})`}
              >
                <rect
                  ref={(el) => {
                    if (el) rectRefs.current[oi] = el;
        ...
        {deviceReference.has_boot_button && (
          <>
            ...
            />
            <g transform="translate(0 -226.483)" onClick={()=>setLoading(true)}>
              <ellipse
                cx={462.589}
                cy={220.435}
                rx={11.957}
    ...
    <strong className="text-red-500 text-xl font-medium">
      No Suitable device
    </strong>
  );
}
